...
- overlay 描边几何（Bug-006）：`overlayBorderBox` 四边外移 2px 使描边内沿贴合
  区域外沿、区域内部无描边像素（几何断言）；dpr 缩放后外移量仍为 CSS 2px、
  物理贴边换算后正确识别为贴边；区域贴左/上/右/下边时各边独立 clamp 到窗口内
  （贴边侧描边内缩，元素整体不越出视口）；未知视口不 clamp；`OverlayFrame`
  渲染断言描边矩形几何、边框/标签颜色与尺寸标签 `-top-6` 不与新描边重叠；
  多框与单框共用同一 `OverlayFrame` 渲染路径。
...
17. 多框会话运行态跨窗口同步依赖两层事件：`frontend_multibox_started`/
    ...
    运行态，不依赖后端修复。
18. overlay 边框外移：选区方框的描边矩形向外扩张 2 CSS px（描边内沿贴合
    区域外沿），使描边像素全部落在捕获区域之外，不再被显示器级捕获
    （WGC 无窗口排除，每帧捕获含 overlay 自身）拍入后经 OCR/翻译污染结果；
    描边不进入捕获区域。各边独立 clamp：区域某边距窗口边不足 2px 时外移量
    相应内缩，完全贴边（x/y 为 0 或区域右/下沿贴显示器边）时该侧描边以
