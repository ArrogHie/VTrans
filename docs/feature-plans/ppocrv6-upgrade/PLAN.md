...
- 需求来源：用户 2026-08-07 提出；参考文档 `docs/PP-OCRv6_small_ONNX_Rust_TS_接入指南.md`（当前未跟踪，需随功能纳入版本库）
...
|------|------|----------|------|----------|------|
| 1 | 08 vtrans-models | 修改（schema + manifest + 脚本 + 文档） | — | `feat/08-ppocrv6-models` | 待派单 |
| 2 | 05 vtrans-ocr | 修改（预处理/后处理/字典/测试/文档） | 依赖 1（schema 字段） | `feat/05-ppocrv6-ocr` | 待派单 |
| 3 | 整合（协调者） | 合并 + workspace 验证 + 端到端 + 报告 | 依赖 1、2 | main 上整合 | 待整合 |
...
- 07 vtrans-translation、09 vtrans-pipeline：不受影响

## 契约变更

- **冻结契约（vtrans-core）**：不涉及。`OcrProvider` trait、`OcrOptions`、`OcrResult`、`OcrError` 全部不变。
- **vtrans-models schema（08 内部契约，需评审并通知下游 05）**：
  - `PreprocessParams` 扩展**可选字段**（serde default，manifest version 保持 1、向后兼容）：
    - det：`box_threshold=0.45`、`max_candidates=3000`、`min_box_size=3`
    - rec：`input_height=48`、`input_width=320`、`append_space=true`、`blank_index=0`
  - 现有字段随新 manifest 更新：`det_threshold` 0.3 → 0.2、`unclip_ratio` 2.0 → 1.4
  - 字典：`ppocrv6_dict.txt`（约 18,708 行 + blank + space ≈ 18,710 类）；ja / en / auto 三个槽位共用同一字典
...
1. 08 派单：`feat/08-ppocrv6-models`（schema 扩展 + 完整脚本 + manifest 回填）
2. 08 合并到 main → 05 从 main 拉分支 `feat/05-ppocrv6-ocr`
3. 05 合并 → 整合验证（workspace 门禁 + 端到端 + 回归）→ 整合报告 → 台账关闭
