...
| 类型 / 函数 | 用途 |
|------|------|
| `AppConfig` | 配置根结构（capture / ocr / translation / result_window / floating_ball / hotkeys / log_level / model_dir / translation_boxes / max_boxes / warning_threshold / version） |
| `CaptureConfig` | 采集：`interval_ms`、`difference_threshold` |
| `OcrConfig` | OCR：`language`、`min_confidence` |
| `TranslationConfig` | 翻译：provider（`openai` / `deepl` / `google` / `azure` / `baidu` / `local`，默认 `openai`）、`region`（Azure）、`app_id`（百度）、`quality`（`"fast"` / `"balanced"`，默认 `"fast"`）、语言对、超时、API endpoint/model、重试次数 |
...
| `TranslationBoxConfig` | 翻译框配置：`id: u32`、`region: ScreenRegion`（来自 vtrans-core）、`color: String`（hex 色值，如 `"#FF6B6B"`） |
| `BOX_COLOR_PALETTE` | 默认颜色调色板（8 种高对比度颜色），[`AppConfig::next_box_color`] 自动分配下一个可用颜色 |
| `ConfigManager` | 配置持久化入口 |
| `ConfigError` | 错误枚举（NotFound / Parse / Validation / Io / UnsupportedVersion） |
| `CURRENT_CONFIG_VERSION` | 当前 schema 版本（`6`） |
...
impl AppConfig {
    ...
    /// 返回调色板中下一个未被使用的颜色；全部用完后循环分配。
    pub fn next_box_color(&self) -> &'static str;
    /// 返回下一个可用 box id（max_id + 1，空列表返回 0）。
...
serde 表示（跨 JSON / IPC 边界）：`Language` 序列化为字符串 `auto` / `zh-CN` / `ja` / `en`；`translation.quality` 为字符串 `"fast"` / `"balanced"`；`translation.region` / `translation.app_id` 为字符串或 `null`（未配置）；`model_dir` 为字符串或 `null`；`version` 为整数；`floating_ball` 为 `{"enabled": bool, "opacity": f64, "size_px": u32}` 对象；反序列化时未知字段被忽略（前向兼容）。完整字段规格见 `docs/modules/02-config.md`。
...
- **版本迁移**：v0（无 `version`）→ v1 仅补版本号；v1 → v2 补齐 `result_window.opacity`、`result_window.font_size_px` 与 `floating_ball` 字段；v2 → v3 补齐 `floating_ball.opacity` / `floating_ball.size_px`——缺失字段由 `serde(default)` 兜底（`0.95` / `14` / `false` / `1.0` / `48`），已存在的字段原样保留；v3 → v4 补齐 `translation.quality`（默认 `"fast"`）并强制 `translation.source_language = ocr.language`（以 OCR 语言为权威，修复历史配置两字段不一致）；v4 → v5 将旧 `provider == "api"` 重命名为 `"openai"`，`region` / `app_id` 缺省补 `None`（显式值保留）；迁移结果写回磁盘。迁移幂等：v5 配置重复迁移无副作用。 v5 -> v6 新增 `translation_boxes`（空列表）、`max_boxes`（`8`）、`warning_threshold`（`4`）——缺失字段由 `serde(default)` 兜底，迁移函数仅补版本号；迁移幂等。

Provider 字段必需性（`validate_translation`）：所有云端 Provider（`openai` / `deepl` / `google` / `azure` / `baidu`）要求 `api_endpoint` 为 `http(s)://` URL；`api_model` 仅 `openai` 必填（DeepL / Google 可选，Azure / 百度可空）；`region` 可选但非空（Azure）；`app_id` 仅 `baidu` 必填。`local` 忽略 endpoint / model / region / app_id。 多框校验（`validate_multi_box`）：`max_boxes` 范围 `1..=32`；`warning_threshold` 不超过 `max_boxes`（`0` 禁用警告）；`translation_boxes` 数量不超过 `max_boxes`；box `id` 唯一；`region` 非零维度（复用 `ScreenRegion::is_valid`）；`color` 为合法 `#RRGGBB` hex 色值。
...
| 手写 v5 配置使用旧 `provider = "api"` 被拒 | `"api"` 已不在白名单；旧文件走 v4→v5 迁移自动改名，手写文件请用 `"openai"` |
| `provider = "baidu"` 但未填 `app_id` 保存被拒 | 百度 APP ID 非敏感、存配置（Secret 存凭据库）；为 `baidu` 配置 `translation.app_id` |
...
| v4→v5 由迁移函数把 `"api"` 改名为 `"openai"` | 配置域 id 与 Provider 一一对应（用户决策），旧文件自动升级 | 保留 `"api"` 作为别名（两套 id 并存，增加前端映射与校验分支） |
| `region` / `app_id` 存配置、API Key / Secret 不落配置 | 前者非敏感且 UI 需要回显；后者归 `vtrans-security` 凭据库，缩小泄露面 | 全部走凭据库（前端无法回显 Azure 区域 / 百度 APP ID） |
...
| 多框字段直接加在 `AppConfig` 上而非独立 `MultiBoxConfig` | 与现有 schema 风格一致（flat fields），避免嵌套 | 独立 `MultiBoxConfig` 子结构（额外嵌套层级） |
| `TranslationBoxConfig` 手动实现 `PartialEq` | `ScreenRegion` 在 vtrans-core 未派生 `PartialEq`，无法 derive | 修改 vtrans-core（冻结禁止）或 `#[derive(PartialEq)]`（编译错误） |
| 颜色调色板提供 8 种高对比度色 | 足够区分常见使用场景（max_boxes=8），颜色循环避免超出调色板后冲突 | 动态生成颜色（复杂、可能不可区分） |
| `color` 用 `String` 存储 hex 色值 | 前端可直接使用（CSS），serde 表示稳定 | 定义 `Color` 枚举/类型（额外映射） |
...
- `translation.region` / `translation.app_id` 仅负责持久化与必需性校验；各 Provider 的鉴权与请求组装（Azure 区域头、百度 MD5 签名等）由 `vtrans-translation` 实现。缓解：07 模块按配置字段组装 Provider。
- 并发 `save` 无序（last-writer-wins）。缓解：高频写场景用 `update` 或调用方自行串行化。
- `translation_boxes` 仅持久化配置；运行时多框翻译编排（并发捕获/OCR/翻译）由 `vtrans-pipeline` 负责。缓解：09 模块按 box_id 隔离状态。
- 颜色调色板固定 8 种；超过调色板大小后颜色循环复用。缓解：修改 `BOX_COLOR_PALETTE` 常量并同步规格文档。
