...
| `CURRENT_CONFIG_VERSION` | 当前 schema 版本（`3`） |
...
serde 表示（跨 JSON / IPC 边界）：`Language` 序列化为字符串 `auto` / `zh-CN` / `ja` / `en`；`model_dir` 为字符串或 `null`；`version` 为整数；`floating_ball` 为 `{"enabled": bool, "opacity": f64, "size_px": u32}` 对象；反序列化时未知字段被忽略（前向兼容）。完整字段规格见 `docs/modules/02-config.md`。
...
- **版本迁移**：v0（无 `version`）→ v1 仅补版本号；v1 → v2 补齐 `result_window.opacity`、`result_window.font_size_px` 与 `floating_ball` 字段；v2 → v3 补齐 `floating_ball.opacity` / `floating_ball.size_px`——缺失字段由 `serde(default)` 兜底（`0.95` / `14` / `false` / `1.0` / `48`），已存在的字段原样保留；迁移结果写回磁盘。
