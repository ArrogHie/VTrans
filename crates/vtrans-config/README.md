...
| `CURRENT_CONFIG_VERSION` | 当前 schema 版本（`5`） |
...
Provider 字段必需性（`validate_translation`）：所有云端 Provider（`openai` / `deepl` / `google` / `azure` / `baidu`）要求 `api_endpoint` 为 `http(s)://` URL；`api_model` 仅 `openai` 必填（DeepL / Google 可选，Azure / 百度可空）；`region` 可选但非空（Azure）；`app_id` 仅 `baidu` 必填。`local` 忽略 endpoint / model / region / app_id。
